...
const getAccessToken = async () => {
	...
	try {
		...
		console.error('Something happend trying to get the access token', err)
	}
}
...
const fetchCalendarEvents = async (days: number) => {
...
export const getCalendarEvents = async (days: number) => {
	try {
		return await fetchCalendarEvents(days)
	} catch(err) {
